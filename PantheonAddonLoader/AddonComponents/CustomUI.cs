...
public class CustomUI : ICustomUI
{
    ...
    private static UIWindowPanel SetupCustomWindow(string windowName, Transform midPanel, Vector2 initialSize)
    {
        ...
        var buttonToCopy = tutorialPopup == null ? null : tutorialPopup.transform.GetChild(0);
        var imageToCopy = tutorialPopup == null ? null : tutorialPopup.GetComponent<Image>();
        ...
        image.color = new Color(0.03f, 0.04f, 0.05f, 0.86f);
        ...
        if (buttonToCopy != null)
        {
            ...
            buttonComponent.onClick.AddCall(new InvokableCall(new Action(() =>
            {
                var audio = buttonCopy.GetComponent<UI_Audio_Function>();
                if (audio != null)
                {
                    audio.Play_UI_Generic_Click();
                }
            })));
        }
        else
        {
            MelonLogger.Msg($"Custom UI window '{windowName}' was created without a close button because no tutorial popup template was found.");
        }
